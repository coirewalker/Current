class JamBase
	...
	def self.upcoming_events(no_of_days, zip)
  	...
  	# location search:
		...
		@events.map do |event|
			...
			#gets first song by artists who have songs that !== nil
			artists = event["Artists"].map do |artist|
				...
				unless @artist
					@artist = Artist.create(name: artist["Name"], jambase_id: artist["Id"]) 
						#venue: venue["name"]
						#must create artist DB column t.string "venue_name" for above.  
	...
	def self.fake_events
	 ...
	 # 	data["Events"].each do |dat|
		# 	track[:venue_name] = dat["Venue"]["Name"]
		# 	dat["Artists"].each do |artist|
 	# 			track[:soundcloud_id] = artist["songs"]["soundcloud_id"]
		# 	end 
		# 	my_data << track
		# end 
		# return my_data
